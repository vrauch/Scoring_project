...
from analysis_modules import (
    ...
    save_analysis_result, build_backlog
)
import os
import contextlib
...
if rows:
    ...
    for row in rows:
        ...
        prompt = f"""
                    Analyze the following text for alignment with the specified criteria based on the context of the project focus.
                    Text: {assessment}
                    Criteria: {criteria}
                    Focus: {project_name}

                    1. Determine whether the alignment is weak, moderate, or strong.
                    2. Provide a brief, fact-based synopsis (less than 100 words) justifying the alignment rating. 
                    Focus on specific points of alignment or gaps relative to the criteria.
        ...
        prompt = f"""
                    Analyze the following scenario regarding the progression from Level 1 to Level 2 maturity within the 
                    context of the project focus.
                    Alignment at Level 1: {alignment}
                    Criteria for Level 2 Maturity: {criteria2}
                    Focus: {project_name}

                    1. Based on the Level 1 Alignment and Level 2 maturity criteria, write a short, fact-based 
                    recommendation synopsis (no more than 100 words) that supports this progression. 
        ...
        prompt = f"""
            ...
            3. Activity 3
         """
        backlog = build_backlog(prompt, recommendation)
        backlog = clean_and_normalize_text(backlog)
