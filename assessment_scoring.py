...
def ensure_nltk_data(package_name, resource_path):
    try:
        # Check if the package is already available
        nltk.data.find(resource_path)
    except LookupError:
        # If not available, download the package and suppress output
        with contextlib.redirect_stdout(open(os.devnull, 'w')):
            nltk.download(package_name)


# Check for 'punkt' and 'stopwords', and download if necessary (suppressing output)
ensure_nltk_data('punkt', 'tokenizers/punkt')
ensure_nltk_data('stopwords', 'corpora/stopwords')
...
rows = analysis_modules.load_from_db_project(project_id)
